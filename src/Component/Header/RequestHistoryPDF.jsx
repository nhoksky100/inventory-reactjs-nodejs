...
// class RequestHistoryPDF extends Component {
//     constructor(props) {
//         super(props);
//         this.state = {
//             dataTeamp: null,
...
//             const [dataPDFRequest, dataPDFTransferHistory, dataRequest, dataRequestTransfer, dataTransferExportApprove] = await Promise.all([
...
//             if (dataPDFRequest && dataPDFTransferHistory) {
//                 if (this._isMounted) {
//                     const mergedData = [...dataPDFRequest, ...dataPDFTransferHistory];
//                     this.sortByDate(mergedData);  // Sắp xếp mảng hợp nhất theo ngày
//                     this.setState({
//                         dataMergedPDF: mergedData
//                     });
//                 }
//             }
...
//                 if (this._isMounted) {
//                     this.setState({
//                         dataTransferExportApprove: dataTransferExportApprove,
...
//     checkDataIdHistory = (idHistory) => {
//         const { dataPDFTransferHistory, dataPDF } = this.state;

//         let matchedIdHistory = null;
//         let matchedIdHistorys = []; // Initialize an array to store the matched idHistorys

//         // Lặp qua từng phần tử trong dataPDF để tìm idHistory trùng khớp
//         dataPDF.forEach((request) => {
//             const requestIdHistory = request.requestIdHistory;


//             if (requestIdHistory !== null) {
//                 // Lặp qua từng phần tử trong dataPDFTransferHistory để so sánh idHistory
//                 dataPDFTransferHistory.forEach((history) => {
...
//     // xuất tệp fdf khi chuyển kho thành công

//     handleExportPDF = (caseRequest,idRequests) => {
//         switch (parseInt(caseRequest)) {
//             case 0:
//                 this.exportPDFTransferApproved(idRequests)
//                 break;

//             case 1:
//                 this.exportPDFTransfer(idRequests)
//                 break;

//             case 2:
//                 this.exportPDFInto(idRequests)
//                 break;
...
//         // caseRequest 2
//         const { idRequestTeamp, dataRequest } = this.state;
...
//     showFormRow = () => {
//         const { dataMergedPDF, idRequestTeamp } = this.state;
//         if (dataMergedPDF.length > 0) {
//             const currentTodos = this.currentTodos(dataMergedPDF)
//             return currentTodos.map((value, key) => {
//                 let caseRequest = value.caseRequest || 2
//                 let idRequests = value.idRequests || value.idRequestTransfers;
//                 idRequests = idRequests !== null ? idRequests.split(', ') : []

//                 return (
//                     <tr key={key}>

//                         <td>{value.idHistory || value.idHistory}</td>
//                         <td>{idRequests}</td>

//                         <td>{parseInt(caseRequest) !== 2 ? 'Xuất' : 'Nhập'}</td>
//                         <td>{value.dateCreated}</td>
//                         <td>
//                             <img style={{ width: '50px', cursor: 'pointer' }} onClick={() => this.handleExportPDF(caseRequest, idRequests)} title='Xuất file PDF' src='../icons/color/pdf-download.png' />
...
//             dispatch({ type: 'dataSearch', action_dataSearch })
//         },


//     }
// }
// export default connect(mapStateToProps, mapDispatchToProps)(RequestHistoryPDF)
