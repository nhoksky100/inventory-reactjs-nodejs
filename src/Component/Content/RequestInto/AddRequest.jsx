...
class AddRequest extends Component {
    ...
    getData = async () => {
        ...
        try {
            ...
            const [dataRequest, dataItemList, dataListAccount, dataRequestTeamp, dataMember, dataWarehouse] = await Promise.all([
                getdataRequest(),
                getdataItemsList(),
                getdataListAccount(),
                getDataRequestTeamp(),
    ...
    handleSave = () => {
        const { permission, newRowDataList, newRowDataListTeamp, rowAddIndex, countNewRow, memberName, memberDepartment, dataRequest } = this.state;
        const promises = [];
        ...
        if (this._isMounted) {
            newRowDataList.forEach(newRowData => {
                ...
                if (orderName && unit && amount && unitPrice && intoMoney && warehouseAreaName) {
                    ...
                    // Kiểm tra xem có orderCode nào trong dataRequest giống với orderCode trong newRowData không
                    const existingData = dataRequest.find(request => request.orderCode === orderCode);
                    ...
                    const promise = axios.post('/addOrderRequest', {
                        ...
                        if (permission !== 'Thành viên' && permission !== 'Thành viên kho') {
...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        getValueIndex: (action_rowIndex) => {
            dispatch({ type: 'valueIndex', action_rowIndex })
        },
