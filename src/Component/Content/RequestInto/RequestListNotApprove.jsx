...
class RequestListNotApprove extends Component {
    ...
    checkPermissionApprove = (pointApprove, value) => {
        ...
        // const permissions = {
        //     'Trưởng phòng': [parseInt(pointApprove[1])],
        //     // 'Trưởng phòng': [parseInt(pointApprove[2])],
        //     'Lãnh đạo': [parseInt(pointApprove[3])]
        // };

        // console.log(pointApprove[0]);
        // if (permissions.hasOwnProperty(permission)) {
        //     if (permission === 'Trưởng phòng' && departmentApproveDate === 'Kế toán') {
        //         return permissions['Trưởng phòng kế toán'].includes(1);
        //     }
    ...
    handleUpdateApproved = (value, pointApprove) => {
        const { dataApproveDate, departmentApproveDate, memberName, reasonMessage, idApproveDate } = this.state;
        ...
        if (flagNotApprove) {
            ...
            axios.post('/updateAppremovedRequest', { id, orderSupplierName: value.orderSupplierName, orderPointApprove, statusOrder, dateUpdate, reasonMessage, orderComplete }).then((res) => {
                ...
                toast(<div className="advertise"><i className="fa fa-minus-circle" aria-hidden="true" />
                    <i>Duyệt đơn không thành công!</i></div>);
            });
        }
    }

    handleUpdateApproveReturn = (value, pointApprove) => {
        const { dataApproveDate, departmentApproveDate, memberName, idApproveReturn, reasonMessage } = this.state;
        // const permissionIndex = this.state.permissionMapping[this.state.permission];
        // let flagApprove = false, flagNotApprove = false;
        const { permission } = this.state;
        ...
        axios.post('/updateAppremovedRequest', { id, orderSupplierName: value.orderSupplierName, orderPointApprove, 
            statusOrder, dateUpdate, reasonMessage, orderComplete  }).then((res) => {
            toast(<div className="advertise"><i className="fa fa-minus-circle" aria-hidden="true" />
